...
function PopperButton({children,menus,name,checkTypeSensor}) {
  ...
  const ListMenu = ({ dept, data, hasSubMenu,type, menuName}) => (
    <div className='max-h-40 overflow-auto '  >
      <div  onClick={() => handleArrowClick(menuName,data,type)}  
            dept={dept}>
        <button className={" "+(dept==1?"ml-4":" ml-8") +(dept===2?" hover:font-bold":"")
      } >
  ...
  const SubMenu = ({ dept, data, toggle,type }) => {
    ...
    render={attrs => (
     <div className="flex  flex-col w-48 max-h-45 bg-slate-100 border-2" tabIndex="-1" {...attrs}>
      {
  ...
  >
   <button  className=" boder-2 w-28 px-2 py-1 mr-2 rounded-full hover:bg-red-600 hover:text-gray-50 bg-slate-50">{children} <ChevronRightIcon/></button>
    </Tippy>
    </div>
}
